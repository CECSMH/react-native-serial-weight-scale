...
class ToledoHandler(model: String?) : BaseHandler("toledo", model ?: "general") {
    ...
    override fun readWeight(): Double {
        if (model == "ti420") {
            ...
            throw UnstableWeightException("Failed after 5 attempts")
        }
...
class ToledoHandler(model: String?) : BaseHandler("toledo", model ?: "general") {
    ...
    override fun parseResponse(response: String): Double {
        ...
        when (model) {
            "prix3", "bcs21" -> {
                when {
                    response.startsWith("${Constants.STX}D+") -> throw UnstableWeightException("Unstable weight", response)
                    response.startsWith("${Constants.STX}S-") -> throw NegativeWeightException("Negative weight", response)
                    response.startsWith("${Constants.STX}S+") -> {
                        if (response.length < 12) throw InvalidResponseException("Invalid length", response)
                        val weight_str = response.substring(3, 11).trim()
                        return ParserUtils.parseWeight(weight_str, 3)
                    }
                    response.contains("s") -> throw OverloadException("Overload", response)
                    response.contains("z") -> throw ZeroCaptureException("Zero capture", response)
                    response.contains("c") -> throw CalibrationException("Calibration error", response)
                    else -> throw InvalidResponseException("Invalid format", response)
                }
            }
            "ti420" -> {
                if (response.contains("#96")) {
            ...
            else -> throw InvalidResponseException("Unsupported model: $model", response)
        }
    }
